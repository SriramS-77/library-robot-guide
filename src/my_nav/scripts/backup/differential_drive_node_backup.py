...
# -------------------------
class RobotConfig:
    WHEEL_DIAMETER_M = rospy.get_param('~wheel_diameter_m', 0.065)   # meters
    WHEEL_RADIUS_M = WHEEL_DIAMETER_M / 2.0
    WHEEL_BASE_M = rospy.get_param('~wheel_base_m', 0.15)          # distance between wheels (m)
    MAX_ROBOT_SPEED = rospy.get_param('~max_robot_speed', 1.12)     # max linear robot speed (m/s)
    ...
    MAX_WHEEL_LINEAR_SPEED = rospy.get_param('~max_wheel_speed', MAX_ROBOT_SPEED)
    # Acceleration limits (m/s^2 for linear wheel speed)
    ACC_LIN = rospy.get_param('~acc_lim_lin', 0.5)     # m/s^2
    ACC_ANG = rospy.get_param('~acc_lim_ang', 1.0)     # rad/s^2
    CONTROL_RATE = rospy.get_param('~control_rate', 20) # Hz
    CMD_TIMEOUT = rospy.get_param('~cmd_timeout', 0.5)  # seconds, stop if no cmd_vel
    USE_GPIO = rospy.get_param('~use_gpio', True)       # set False for testing without hardware
    MOTOR_PINS = rospy.get_param('~motor_pins', {
        'left_forward': 23,
        'left_backward': 22,
        'right_forward': 27,
        'right_backward': 17,
...
class DifferentialDriveNode:
    def __init__(self):
        ...
        # Params
        self.wheel_base = RobotConfig.WHEEL_BASE_M
        self.wheel_radius = RobotConfig.WHEEL_RADIUS_M
        self.max_wheel_speed = RobotConfig.MAX_WHEEL_LINEAR_SPEED
        self.acc_lin = RobotConfig.ACC_LIN
        self.acc_ang = RobotConfig.ACC_ANG
        self.rate_hz = RobotConfig.CONTROL_RATE
        self.cmd_timeout = RobotConfig.CMD_TIMEOUT
        ...
        # Motor controller
        if RobotConfig.USE_GPIO and GPIOZERO_AVAILABLE:
            rospy.loginfo("Using gpiozero Motor controller")
            self.motor = GpioMotorController(RobotConfig.MOTOR_PINS, self.max_wheel_speed)
        else:
            if RobotConfig.USE_GPIO and not GPIOZERO_AVAILABLE:
        ...
        # Internal wheel command state (linear m/s)
        self.cmd_left = 0.0
        self.cmd_right = 0.0
        ...
        while not rospy.is_shutdown():
            ...
            with self.lock:
                ...
                if time_since_cmd > self.cmd_timeout:
                    ...
                    tgt_v = max(min(self.target_v, RobotConfig.MAX_ROBOT_SPEED), -RobotConfig.MAX_ROBOT_SPEED)
                    tgt_w = self.target_w
                ...
                try:
                    ...
                    rospy.logerr_throttle(5.0, f"Motor command failed: {e}")
