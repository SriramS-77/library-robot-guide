...
class DifferentialDriveNode:
    def __init__(self):
        ...
        self.max_robot_speed = rospy.get_param('~max_robot_speed', 1.00)     # max linear robot speed (m/s), 1.12
    ...
    def spin(self):
        ...
        while not rospy.is_shutdown():
            ...
            with self.lock:
                ...
                rospy.loginfo(f"Initial Speed Left: {self.cmd_left}, Right: {self.cmd_right}...")

                MIN_SPEED = 0.4

                if 0 < self.cmd_left < MIN_SPEED:
                    self.cmd_left = MIN_SPEED
                elif -MIN_SPEED <= self.cmd_left < 0:
                    self.cmd_left = -MIN_SPEED

                if 0 < self.cmd_right < MIN_SPEED:
                    self.cmd_right = MIN_SPEED
                elif -MIN_SPEED <= self.cmd_right < 0:
                    self.cmd_right = -MIN_SPEED
                ...
                try:
                    ...
                    rospy.loginfo(f"Final Speed Left: {self.cmd_left}, Right: {self.cmd_right}...")
